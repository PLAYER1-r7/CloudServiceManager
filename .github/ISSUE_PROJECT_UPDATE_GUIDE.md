...
- ソフト導入後は `.devcontainer/` の設定へ反映する（必要に応じて Dockerfile、`devcontainer.json`、セットアップスクリプトを更新）
- 認証が必要な操作は、実行前に必ず認証をリクエストする（例: `gh auth login`, クラウドCLI認証）
- 未認証状態で権限を要する操作を継続しない。認証確認後に再開する
