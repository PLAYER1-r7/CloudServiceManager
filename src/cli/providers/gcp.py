...
class GCPProvider:
    ...
    def __init__(
        self, auth: Optional[GCPAuth] = None, project_id: Optional[str] = None
    ):
        ...
        self.instances_client = compute_v1.InstancesClient(
            credentials=self.auth._credentials
        )
    ...
    def list_services(self, region: Optional[str] = None) -> List[CloudService]:
        ...
        try:
            ...
            if region:
                ...
                logger.info(
                    f"Fetching Compute Engine instances from {len(zones)} zones"
                )
    ...
    def get_service(
        self, service_id: str, region: Optional[str] = None
        ...
        try:
            ...
            for zone in zones:
                try:
                    ...
                    logger.error(
                        f"Error fetching instance {service_id} from zone {zone}: {e}"
                    )
                    raise
    ...
    def _convert_to_cloud_service(
        self, instance: compute_v1.Instance, zone: str
    ) -> CloudService:
        """
